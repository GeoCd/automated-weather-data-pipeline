...
# ------------------------------------------------------------------------------------------------------------
# All stations workflow
# ------------------------------------------------------------------------------------------------------------
def run_all_stations(mode: str = None) -> None:
    """
    Iterates all Registered=1 and Status=ONLINE stations from the CSV
    and runs the workflow for each one.
    """
    ...
    with open(stations_csv, newline='', encoding='utf-8') as f:
        ...
        stations = [
            ...
            if row.get('Status', 'OFFLINE').strip().upper() == 'ONLINE'
            and row.get('Registered', '0').strip() == '1'
        ]

    print(f"Running workflow for {len(stations)} registered ONLINE stations.")

    for row in stations:
        station_id = row['Estacion'].strip()
        print(f"\n{'='*50}")
        print(f"Processing: {station_id} ({row['Ciudad']})")
        try:
            run_workflow(id=station_id, mode=mode)
        except Exception as e:
            print(f"[{station_id}] ERROR: {e}")
            failed.append({'station': station_id, 'error': str(e)})

    print(f"\n{'='*50}")
    print(f"All stations processed. Failures: {len(failed)}")
    for f in failed:
